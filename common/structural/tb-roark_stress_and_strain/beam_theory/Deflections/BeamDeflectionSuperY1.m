...
%
%           An (n x p) matrix of values necessary for calculating the
...
%    
    if iscellstr(beamMethod) && size(Yvars, 1) == size(beamMethod, 1)
        ...
        for i = 1:length(beamMethod)
            Def = Def + BeamDeflectionY1(Ivars, Yvars{i,1}(1,:), E, x, IMethod, char(beamMethod(i,1)));
        end

    elseif ischar(beamMethod)

        Def = sum(BeamDeflectionY1(Ivars, Yvars, E, x, IMethod, beamMethod),1);
