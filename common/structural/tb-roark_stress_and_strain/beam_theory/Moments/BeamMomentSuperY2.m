function Mom = BeamMomentSuperY2(Yvars, Ivars, E, x, IMethod, beamMethod)
...
%
    if iscellstr(beamMethod) && size(Yvars, 1) == size(beamMethod, 1)
        ...
        for i = 1:length(beamMethod)
            Mom = Mom + BeamMomentY2(Ivars, Yvars{i,1}(1,:), E, x, IMethod, char(beamMethod(i,1)));
        end

    elseif ischar(beamMethod)

        Mom = sum(BeamMomentY2(Ivars, Yvars, E, x, IMethod, beamMethod),1);
