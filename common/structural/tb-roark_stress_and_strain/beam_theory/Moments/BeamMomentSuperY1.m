function Mom = BeamMomentSuperY1(Yvars, Ivars, E, x, IMethod, beamMethod)
% BeamMomentSuper: Function for calculating the moments in a beam using the
% superposition of two or more loads
%
% Input:
%
...
%           beam moments according to the corresponding string in the cell
...
%                method by which the beam moment is to be calculated.
...
%
    if iscellstr(beamMethod) && size(Yvars, 1) == size(beamMethod, 1)
        ...
        for i = 1:length(beamMethod)
            Mom = Mom + BeamMomentY1(Ivars, Yvars{i,1}(1,:), E, x, IMethod, char(beamMethod(i,1)));
        end

    elseif ischar(beamMethod)

        Mom = sum(BeamMomentY1(Ivars, Yvars, E, x, IMethod, beamMethod),1);
