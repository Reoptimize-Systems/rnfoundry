function Slope = BeamSlopeSuperY1(Yvars, Ivars, E, x, IMethod, beamMethod)
% function: BeamSlopeSuperY1
%
...
% Output:
%
%   Slope - (n x i) column vector of values of the slope of a beam, at
...
%    
    if iscellstr(beamMethod) && size(Yvars, 1) == size(beamMethod, 1)
        ...
        for i = 1:length(beamMethod)
            Slope = Slope + BeamSlopeY1(Ivars, Yvars{i,1}(1,:), E, x, IMethod, char(beamMethod(i,1)));
        end

    elseif ischar(beamMethod)

        Slope = sum(BeamSlopeY1(Ivars, Yvars, E, x, IMethod, beamMethod),1);
