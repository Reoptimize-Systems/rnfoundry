function Slope = BeamSlopeSuperY2(Yvars, Ivars, E, x, IMethod, beamMethod)
...
%    
    if iscellstr(beamMethod) && size(Yvars, 1) == size(beamMethod, 1)
        ...
        for i = 1:length(beamMethod)
            Slope = Slope + BeamSlopeY2(Ivars, Yvars{i,1}(1,:), E, x, IMethod, char(beamMethod(i,1)));
        end

    elseif ischar(beamMethod)

        Slope = sum(BeamSlopeY2(Ivars, Yvars, E, x, IMethod, beamMethod),1);
