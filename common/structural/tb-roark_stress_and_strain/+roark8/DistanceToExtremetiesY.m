function y_c = DistanceToExtremetiesY (IVars, IMethod)
% calculates the maximum distance from the neutral axis to the extremeties
% of a section in the Y direction (i.e. distance from neutral axis parallel
% to x axis)
%
% Syntax
%
% y_c = DistanceToExtremetiesY (IVars, IMethod)
%
% Input:
%
...
%     extremeties in the y direction, calculated according to 'IMethod'
%
%
...
%             y_c = Table1r6I_1(IVars);
        ...
        case {'A1.15', 'SolidCircle', 'Circle'}
            % Solid Circle
            y_c = roark.Sections.DistToExtremities.CircleY (IVars);

        case {'A1.16', 'Annulus', 'HollowCircle'}
            % Hollow Circle
            y_c = roark.Sections.DistToExtremities.HollowCircleY (IVars);
...
% 
        case 'polygon'
            ...
            for ind = 1:size(IVars, 3)
                ...
                y_c(ind,1) = max ( abs (thisvars(:,1) - GEOM(2)) );
            end

        otherwise

            y_c = feval(IMethod, IVars);
