...
public:
  ...
  // getDoubleValue
  // gets value from matrix using matrix subscripts. Note matrix subscripts
  ...
  // gets value from matrix using matrix subscripts. Note matrix subscripts
  // should be zero-based not one-based
  int getInt32Value (std::vector<mwSize> index)
  {
      // check it's  double matrix
      if ( !mxIsClass(wMxArray, "int32") )
      {
          mexErrMsgIdAndTxt("CPP:mxArrayWrapper:notint",
              "Integer value requested for non-int32 matrix (of class %s).",
              mxGetClassName(wMxArray) );
      }
  ...
  void setDoubleValue (std::vector<mwSize> index, double value)
  {
      ...
      {
          mexErrMsgIdAndTxt("CPP:mxArrayWrapper:notdouble",
              "Attempted to set double value in non-double matrix (matrix is of class %s).",
              mxGetClassName(wMxArray) );
      }
