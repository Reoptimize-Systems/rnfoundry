classdef fileDriver < mbdyn.pre.base
% base class for all MBDyn file drivers
%
% Syntax
%
% el = mbdyn.pre.fileDriver ('Parameter', Value)
%
% Description
%
...
%
    ...
    methods
        ...
        function self = fileDriver (varargin)
            ...
            %    fileDriver, in which case it will be used in the 'name'
            %    property of the fileDriver.
            %
            %  'DefaultShape' - optional string which chooses the shape to
            %    use to represent the fileDriver in a plot when an STL file is
            ...
            %    above). 
            %
            %  'DefaultShapeOffset' - optional 3 fileDriver column vector
            %    which sets the offset of the default shape (see
            %    above) from the default initial position. 
            %
            % Output
            %
            %  el - mbdyn.pre.fileDriver
            %
            
            [options, ~] = mbdyn.pre.fileDriver.defaultConstructorOptions ();
...
%                     self.setSize (1);
        ...
        function loadSTL (self, filename, usename)
            % load an STL file which will be used when visualising the fileDriver
            %
            % Syntax
            %
            % mbdyn.pre.fileDriver (self, filename, usename)
            %
            % Description
            %
            % load an STL file which will be used when visualising the
            % fileDriver instead of the default shapes.
            %
            % Input
            %
            %  el - mbdyn.pre.fileDriver object
            %
            ...
            %
            %
            % See Also: mbdyn.pre.fileDriver.node
            %
        ...
        function setSize (self, varargin)
            % set the size of the fileDriver in plots
            %
            % Syntax
            %
            % setSize (el, sx, sy, sz)
            ...
            % Description
            %
            % setSize is used to set the size of the default fileDriver shape
            % for plotting the fileDriver in a figure. This is used when no
            % STL file is avaialable, of the subclassed elemnt does not
            % provide it's own drawing of the fileDriver. The inputs to
            % setSize depend on what the fileDriver's chosen shape is.
            %
            % Input
            %
            %  el - mbdyn.pre.fileDriver object
...
%                         self.shapeParameters(1) = varargin{1};
        ...
        function setColour (self, newcolour)
            % set the colour of the fileDriver in plots
            
            self.drawColour = newcolour;
        end
        
        function hax = draw (self, varargin)
            % plot the fileDriver in a figure
            %
            % Syntax
            %
            % hax = draw (el)
            ...
            % Description
            %
            % The draw method creates a visualisation of the fileDriver in a
            % figure. If an STL file has previously be added to the
            % fileDriver, this will be plotted. Otherwise a standard shape
            % will be used.
            %
            % The draw method tries to be efficient. Each fileDriver is
            ...
            %    fileDriver. If not supplied, a new figure and axes will be
            %    created. The first time the fileDriver is drawn the handle to
            ...
            %    transform matrix), even if the fileDriver does not think it
            %    needs it.
            %
            %  'Mode' - character vector determining the style in which the
            %    fileDriver will be plotted. Can be one of 'solid',
    ...
    methods (Access = protected)
        ...
        function ok = checkCOGVector (self, cog, throw)
            ...
            if ~ok && throw
                error ('Centre of gravity offset must 3 fileDriver numeric column vector, or keyword ''null'' or empty');
            end
