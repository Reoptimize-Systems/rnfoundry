function [ENOB, SNDR, SFDR, SNR, THDdb] = prettyFFT(wave,f_S,maxh,no_annotation,no_plot,baseline)
...
%   -Skyler
    ...
    % GET THD
    THDdb = -db((f2(bin).^2/(sum(f2(harm(2:end)).^2))))/2; % get SNR (f_in / sum(the rest))
            ...
            if(SFDR > -(.13*xx))
                ...
                if(SFDR > -(.25*xx))
                    infostr=...%['ENOB =\newline' num2str(ENOB,4) ' bits\newline\newline' ...
                        ...
                        '\newlineTHD =\newline' num2str(THDdb,4) 'dB'];
                else
                    infostr=...%['ENOB =\newline' num2str(ENOB,4) ' bits\newline\newline' ...
                        ['SNDR = ' num2str(SNDR,4) 'dB\newline' ...
                        'THD =' num2str(THDdb,4) 'dB'];
                end
                if(bin<numbins/2)
...
function out = db (M)
% matlab has a built-in db function, but lets avoid using it for the
% benefit of Octave, which does not have it implemented
