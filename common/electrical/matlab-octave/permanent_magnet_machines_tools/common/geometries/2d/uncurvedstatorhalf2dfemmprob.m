function [FemmProblem, outernodes, coillabellocs, slotinfo] = ...
    uncurvedstatorhalf2dfemmprob (nslots, yslot, ycoil, yshoegap, xyoke, ...
                                  xcoil, xshoebase, xshoegap, xoffset, side, varargin)
% draw a single-sided slotted uncurved stator
%
% Syntax
%
% [FemmProblem, outernodes, slotinfo] = ...
%       axialfluxstatorhalf2dfemmprob(slots, yslot, ypole, ycoil, yshoegap, ...
...
%
	...
	coillabellocs = [];
    slotinfo.InsulationLabelLocations = [];
    ...
    if ~isempty(slotinfo.inslabelloc)
        slotinfo.inslabelloc(:,1) = slotinfo.inslabelloc(:,1) + xoffset;
    end
...
%     if ~isempty(slotinfo.coillabelloc)
%         slotinfo.coillabelloc(:,2) = slotinfo.coillabelloc(:,2) + slotpos(1)/2;
%     end
    ...
    if Inputs.DrawCoilInsulation
        slotinfo.InsulationLabelLocations = [slotinfo.InsulationLabelLocations; 
                                             slotinfo.inslabelloc(1), slotinfo.inslabelloc(2) + slotpos(1) ];
    end
    ...
    for i = 2:numel(slotpos)
        ...
        if Inputs.DrawCoilInsulation
          slotinfo.InsulationLabelLocations = [slotinfo.InsulationLabelLocations; 
                                               slotinfo.inslabelloc(1), slotinfo.inslabelloc(2) + slotpos(i)];
        end
