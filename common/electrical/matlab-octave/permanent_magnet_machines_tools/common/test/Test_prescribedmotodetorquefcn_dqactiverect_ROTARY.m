% Test_prescribedmotodetorquefcn_dqactiverect_ROTARY.m
%
% This script sets up a simulation of a radial flux permanent magnet
% machine with an average model of an active rectifier, using the reduced
% dq representation of the machine, and controlled using field oriented
% control.
%
...
% design.MachineSidePowerConverter.Vdc = 1.1 * (3 * sqrt(6) / pi) * peakemfest_ROTARY (design.FluxLinkagePhasePeak, rpm2omega (RPM), design.Poles/2) / sqrt(2);
% design.MachineSidePowerConverter.Rds = 0.2e-3;
...
simoptions.Isqref = -200.0; % torque reference (sets torque)
simoptions.FOCStartTime = FOCStart;

simoptions.ODESim.SolutionComponents.DQCurrents.SolutionIndices = [1,2];
simoptions.ODESim.SolutionComponents.DQCurrents.InitialConditions = [ 0, 0];

simoptions = simsetup_ROTARY ( design, [], [], ...
                               'EvalFcn', 'prescribedmotodetorquefcn_dqactiverect_ROTARY', ...
                               'PostSimFcn', 'dqresfun_ROTARY', ...
                               'Rpm', RPM, ...
                               'TSpan', TSpan, ...
                               'RampPoles', 20, ...
...
simoptions.ODESim.OutputFcn = 'Test_prescribedmotodetorquefcn_dqactiverect_ROTARY_ouputfcn';
...
yyaxis left
plot(T, results.EMF);
ylabel('DQ Phase EMFs (V)');
yyaxis right
plot(T, Y);
hold on
plot (T([1,end]), [simoptions.Isqref; simoptions.Isqref], ':k');
hold off
ylabel('DQPhase Currents (A)');
xlabel('Time (s)');
legend ('EMFd', 'EMFq', 'Id', 'Iq', 'Isqref');
