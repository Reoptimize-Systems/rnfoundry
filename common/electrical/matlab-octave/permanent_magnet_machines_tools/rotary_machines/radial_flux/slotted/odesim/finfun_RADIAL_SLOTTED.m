...
%     maxflpos = 0.5 + (design.thetas/2 + design.FirstSlotCenter)/design.thetap;
    ...
    elseif design.CoilLayers == 2
        ...
        % a dept of 1 to the flux linkage calc. The reason for this is that
...
function design = makelossfcns_RADIAL_SLOTTED(design)
    ...
    for ind = 1:numel(design.CoreLoss)
        ...
        design.CoreLoss(ind).BMagnitude = max( sqrt(  realpow( design.CoreLoss(ind).Bx, 2 ) ...
                                                    + realpow( design.CoreLoss(ind).By, 2 ) ), [], 2 );
    end 
