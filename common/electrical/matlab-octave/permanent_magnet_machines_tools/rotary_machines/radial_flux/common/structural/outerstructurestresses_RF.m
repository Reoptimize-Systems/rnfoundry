function [Kmat, Fmat] = outerstructurestresses_RF(faeprob, Rsoi, ls, shearforce, radialforce, structdensity, omega, rimlabel)
...
%     end
    ...
    % now apply radial cetrifugal forces to the entire structure
...
function [ innersurfgcells ] = getinnersurface(faeprob, rimlabel, Rsoi, ls)

    % first get the 3D gcells making up the rim of the machine sstructure
    outercells = subset( faeprob.gcells, ...
                         gcell_select(faeprob.fens, faeprob.gcells, struct('label', rimlabel)) );
    ...
    innersurfgcells = subset(bdrygcells, ...
                             gcell_select(faeprob.fens, bdrygcells, ...
                                struct('cylinder', [0, 0, 0, Rsoi + tol, 2 * ls], ...
                                       'invert', false)));

	% strip the rim cells on either edge
    innersurfgcells = subset(innersurfgcells, ...
                             gcell_select(faeprob.fens, innersurfgcells, ...
                                struct('box', [Rsoi, -Rsoi, Rsoi, -Rsoi, 0, ls], ...
                                       'inflate', tol, ...
                                       'invert', false)));
