function [FemmProblem, outermagsep] = slotlessLfemmprob_torus(design, varargin)
...
% magnet machine
    ...
    gapedgenodes(:,2) = gapedgenodes(:,2) * (design.taupcg / (2*design.taupm));
    ...
    for i = 1:Inputs.NStages
        ...
        if i == 1 
            ...
            % create outer node coordinates for the yoke going clockwise
            % around the yoke corners starting from the bottom left
            ...
            yokecoords(:,2) = yokecoords(:,2) * (design.taupcg / (2*design.taupm));
            ...
            % add a circuits for the coils
            FemmProblem = addcircuit_mfemm(FemmProblem, '1', ...
                'TotalAmps_re', real(Inputs.CoilCurrent), ...
                'TotalAmps_im', imag(Inputs.CoilCurrent));
            
            FemmProblem = addcircuit_mfemm(FemmProblem, '2', ...
                'TotalAmps_re', 0, ...
                'TotalAmps_im', 0);
            ...
            % define the block properties of the coil regions
            coil1BlockProps.BlockType = FemmProblem.Materials(end).Name;
            coil1BlockProps.MaxArea = Inputs.AirGapMeshSize;
            coil1BlockProps.InCircuit = '1';
            coil1BlockProps.InGroup = Inputs.CoilGroup;
            
            % define the block properties of the coil region
            coil2BlockProps.BlockType = FemmProblem.Materials(end).Name;
            coil2BlockProps.MaxArea = Inputs.AirGapMeshSize;
            coil2BlockProps.InCircuit = '2';
            coil2BlockProps.InGroup = Inputs.CoilGroup;
            ...
            [FemmProblem, coil1lseginds, coil1lnodeinds, coil1lblockind, coil1lnodeids] = ...
                addrectregion_mfemm(FemmProblem, xcoil, ycoil, design.tc, design.tauco, coil1BlockProps); 

            % link up the top and bottom of the yoke at the coil edges

            % link the bottom of coil 1 to bottom of yoke
            FemmProblem = addsegments_mfemm(FemmProblem, coil1lnodeids(2), yokenodeids(1));
            ...
            % add a rectangular region making up the other half of coil 1
            [FemmProblem, coil1rseginds, coil1rnodeinds, coil1rblockind, coil1rnodeids] = ...
                addrectregion_mfemm(FemmProblem, xcoil, ycoil, design.tc, design.tauco, coil1BlockProps);        

            % the bottom of coil 1 to bottom of yoke
            FemmProblem = addsegments_mfemm(FemmProblem, coil1rnodeids(1), yokenodeids(2));
            
            
            % Now make adjacent winding
            ...
            [FemmProblem, coil2lseginds, coil2lnodeinds, coil2lblockind, coil2lnodeids] = ...
                addrectregion_mfemm(FemmProblem, xcoil, ycoil, design.tc, design.tauco, coil2BlockProps); 

            % link up the top and bottom of the yoke at the coil edges
            
            % the top of coil to top of yoke
            FemmProblem = addsegments_mfemm(FemmProblem, coil2lnodeids(3), yokenodeids(4));
...
%             FemmProblem = addsegments_mfemm(FemmProblem, coilnodeids(2), yokenodeids(1));
            ...
            [FemmProblem, coil2rseginds, coil2rnodeinds, coil2rblockind, coil2rnodeids] = ...
                addrectregion_mfemm(FemmProblem, xcoil, ycoil, design.tc, design.tauco, coil2BlockProps);        

            % link up the top and bottom of the yoke at the coil edges

            % the top of coil to top of yoke
            FemmProblem = addsegments_mfemm(FemmProblem, coil2rnodeids(4), yokenodeids(3));
            
            % the top of coil 1 to bottom of coil 2
            FemmProblem = addsegments_mfemm(FemmProblem, coil1lnodeids(3), coil2lnodeids(2));
            FemmProblem = addsegments_mfemm(FemmProblem, coil1rnodeids(4), coil2rnodeids(1));
            ...
            [FemmProblem, seginds, yokenodeinds, blockind, yokenodeids] = ...
                addrectregion_mfemm(FemmProblem, corexpos, 0, design.ty, 2 * design.taupm * (design.taupcg / (2*design.taupm)), yokeBlockProps, SegProps);
        ...
        labelloc = [gapedgenodes(1,1) + design.g/2, design.taupm*(design.taupcg / (2*design.taupm))];
        ...
        labelloc = [gapedgenodes(1,1) + 3*design.g/2 + 2*design.tc + design.ty, design.taupm*(design.taupcg / (2*design.taupm))];
