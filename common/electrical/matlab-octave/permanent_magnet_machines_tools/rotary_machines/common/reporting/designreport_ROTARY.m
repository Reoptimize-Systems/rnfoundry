...
%
    ...
    if isfield(design, 'ForceGapClosingWithDisp')
...
'\end{table}';}];
    ...
    if isfield (design, 'ForceGapClosingWithDisp') && isfield (design, 'DispGapClosingForce')
        ...
        tabledata = [design.DispGapClosingForce(:) .* 1e3, design.ForceGapClosingWithDisp(:) ./ 1e3];
        
        displaytable(tabledata, ...
                     'ColHeadings', {'Rotor Displacement', 'Gap Closing Force (kN)'}, ...
                     'ColWidth', [20, 20], ...
                     'Format', fms, ...
                     'RowHeadings', rowheadings, ...
                     'FileID', fid, ...
        ...
        closingforecetablestrs = txtfile2cell(fname);
    
        delete (fname);

rotaryreportstrs = [rotaryreportstrs; {...
'\begin{table}[htb]';
'\centerline{';
'\begin{tabular}{ll}'
'\toprule';
'Rotor Displacement (mm) & Gap Closing Force (kN) \\';
'\midrule';
}; ...
closingforecetablestrs;
{...
'\bottomrule';
'\end{tabular}';
'}';
'\caption{Gap closing forces with displacement of the rotor from centre.}';
'\end{table}';}];
