function evaloptions = designandevaloptions_PMSM(evaloptions)
% parses the evaloptions structure for the function
% designandevaluate_PMSM filling in missing values with defaults etc.
%
% Syntax
% 
% evaloptions = designandevaloptions_PMSM(evaloptions)
% 
% Input:
%
...
%             the length of the field and armature. 
    ...
    elseif nargin == 1
        ...
        if ~isfield(evaloptions, 'minRMSemf')
            evaloptions.minRMSemf = 300;
        end
        
        if ~isfield(evaloptions, 'maxJrms')
            evaloptions.maxJrms = 5e6;
        end
