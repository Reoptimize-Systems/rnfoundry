function varargout = systemode_linear(t, x, design, simoptions)
% systemode_linear: solves the right had side of the differential
% equations for a combined linear machine and buoy system
%
% Syntax:
%
...
% 
% [..., Feff] = systemode_linear(t, x, design, simoptions)
% 
...
%
%   Feff - total electromagnetic forces acting on effector at the current
%   time step
%
...
%
    ...
    if nargin == 0
        ...
        varargout{1} = {'ydot', 'EMF', 'Feff', 'xE', 'vE', ...
                        ...
                        'radiation_force_surge', 'dpsidxR', 'wave_height', ...
                        'FaddEBD', 'RPhase'};
        return;
    end
    ...
    Fexternal = FfeaVec + FesVec;
    
    % Determine the buoy/sea interaction forces and derivatives. The last N
    ...
    if nargout > 1
        ...
        % wave height
        varargout{16} = wave_height;
        % extra forces such as end stop force
        varargout{17} = [0,0,0,Fes,0];
        % phase resistance
        varargout{18} = design.PhaseResistance;
