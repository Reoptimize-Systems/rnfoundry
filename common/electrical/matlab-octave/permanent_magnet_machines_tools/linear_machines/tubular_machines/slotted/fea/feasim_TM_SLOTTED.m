function [ RawForce, ...
           ...
           BslotPos, ...
           slotIntB, ...
           design ] = feasim_TM_SLOTTED (design, simoptions, zpos, varargin)
% performs a single finite element simulation of a slotted tubular linear
...
%   BslotPos, ...
%   slotIntB, ...
%   design ] = feasim_TM_SLOTTED (design, simoptions, z, 'Parameter', value)
%
% Inputs 
%
...
%  theta - 
    ...
    if nargin == 0
        RawForce = {@slotintAdata, @slotintBdata, @corelosssetup };
    end
    ...
    [design.FemmProblem, design.FieldDrawingInfo, design.ArmatureDrawingInfo] = ...
                        slottedfemmprob_tubular (design, ...
...
%                                                 gvector);
    ...
    NBpnts = 100;
    y = repmat (design.zs + zpos, 1, NBpnts);
    if strcmpi (design.ArmatureType, 'external')
        x = linspace (design.Rag, design.Ryo, NBpnts);
    elseif strcmpi (design.ArmatureType, 'internal')
        x = linspace (design.Ryi, design.Rag, NBpnts);
    end
...
%                end
    ...
    if Inputs.GatherVectorPotentialData
        ...
        [AslotPos, slotIntA] = slotintAdata_TM_SLOTTED (design, zpos, solution);
...
function design = corelosssetup (design, simoptions, feapos, solution)
    ...
    design = corelosssetup_SLOTTED (design, feapos, groups, solution);

end

function design = corelosssetup_SLOTTED (design, feapos, groups, solution)
    ...
    if isa(solution, 'fpproc')
        ...
        error ('RENEWNET:corelosssetup_SLOTTED:nousefemm', ...
