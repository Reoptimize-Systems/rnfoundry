function [design, simoptions] = finfun_TM_SLOTTED (design, simoptions)
...
%         % (see comment above). This time we add an offse so we're starting
...
%     end
    ...
    if ~isfield (design, 'CoreLossSLMs')
        design = makelossfcns_TM_SLOTTED (design);
    end
    
    % estimate the masses of the components
    design = materialmasses_TM_SLOTTED (design, simoptions);
    
    % estimate the Field inertia (approximating as a hollow cylinder)
%     if (strcmp(design.ArmatureType, 'internal'))
%         design.FieldMomentOfInertia = 0.5 * design.FieldMass * (design.Rmi^2 + design.Rbo^2);
...
%         design.FieldMomentOfInertia = 0.5 * design.FieldMass * (design.Rbi^2 + design.Rmo^2);
...
% creates a cogging force slm    
    ...
    % circumference
    normcoggingTorqueslm = slmengine (design.MagFEASimPositions, design.RawCoggingForce ./ (2 * pi), ...
            'EndCon', 'periodic', ...
    ...
    coggingForce = periodicslmeval ( bsxfun(@plus, pos, skewoffset), normcoggingTorqueslm, 0, false );

    % calculate the total cogging force  
    coggingForce = ((2*pi)/skew(2)) * sum(coggingForce,1);
    
    design.slm_coggingtorque = slmengine (pos, coggingForce, ...
            'EndCon', 'periodic', ...
...
function design = makelossfcns_TM_SLOTTED(design)
