function design = designstructure_ACTM(design, options)
% designstructure_ACTM: chooses the optimum inner shaft radius for a
...
%
    ...
    [PorF, z, actForce, design.maxDefCriteria, netResultantForce, P] = evaluatestructure_ACTM(design, options, Etemp);
    
    % if the coil hoop stress exceeds the maximum acceptable, generate a
    % penalty factor for the design
    ...
    % succeed so we add a design.StructPenalty factor. We will also return a solid shaft which
    % will add extra cost, but we will test this first to see if this would
    % also fail and add extra penalties if this is the case
    ...
    [PorF, z, design.actForce, design.maxDefCriteria, netResultantForce, P] = evaluatestructure_ACTM(design, options, Etemp, netResultantForce, P);
    
    % If the shaft also fails on it's own, add a further design.StructPenalty to account
    % for this
    if PorF(4) > design.defThresh
    ...
    [PorF, z, design.actForce, design.maxDefCriteria, netResultantForce, P] = evaluatestructure_ACTM(design, options, E, netResultantForce, P);
    ...
    if PorF(4) > design.defThresh
        % In this case the design.StructPenalty factor is the factor by which the total
        % deflection exceeds the allowed deflection
        design.StructPenalty(2) = (PorF(4) - design.defThresh) ./ (design.g - design.defThresh);
    end
    ...
    x1 = 0; 
    x2 = 0.95;
    % This will find the RsiVRso value that will give the closest deflection
    % to the deflection threshold
    design.RsiVRso = fminbnd(@(x)defThreshDiff_DesignStructure_ACTM(x, design, options, E, netResultantForce, P), x1, x2);
    design = ratios2dimensions_ACTM(design);
                                       
end


function diff = defThreshDiff_DesignStructure_ACTM(tempRsiVRso, design, options, E,netResultantForce, P)

    design.RsiVRso = tempRsiVRso;
    design = ratios2dimensions_ACTM(design);

    [PorF, z, design.actForce, design.maxDefCriteria] = evaluatestructure_ACTM(design, options, E, netResultantForce, P);
