function FluxDensity = fluxdensityfrompolys_ACTM(WmVWp, WpVRm, RsoVRm, Rm, rzCoords, Polynomials, rSplitPt)
% fluxdensityfrompolys_ACTM: a function which returns the normalised flux density
% above the translator. This is calculated by evaluating the polynomial
% approximations of the flux density at the appropriate positions.
%
% arguments: (input)
...
%              split piecewise
%
%   Polynomials - matrix of polynomials to use for evaluating the flux
%                 density. Will be loaded from disc if not provided. 
%
% Arguments: (output)
%
%   FluxDensity - (n x 2) Matrix of Flux Density values at the coordinates
%                 supplied in the input. The first column is the axial flux
%                 at the given points, the second the radial flux.
%
...
% The University of Edinburgh
    ...
    % (i x 4 x 200) array. The row size varies for each polynomial.
    % The first three columns in each of the 204 polynomials
    % corresponds to a variabledescribing the machine and the
    ...
    polys = selectpolys_TM(rzCoords, rSplitPt);

    % Next the polynomials can be evaluated for the given positions
    % and machine properties. evaluatepolys_ACTM returns an (n x 2)
    % matrix, the first column corresponding to the first column of
    % polys, the second, the second column of polys.

    tempFlux = evaluatepolys_ACTM(WmVWp, WpVRm, RsoVRm, Rm, rzCoords(:,1), polys(:,1:2), Polynomials);
    tempFlux(:,3:4) = evaluatepolys_ACTM(WmVWp, WpVRm, RsoVRm, Rm, rzCoords(:,1), abs(polys(:,3:4)), Polynomials);

    % The polarity of some of the radial flux density values may need to be
    % flipped, i.e. if position z = 1 is required, position z=0 is
    % calculated and then we must flip it.
    polarityMask = (polys(:,3:4) < 0) * -1 + (polys(:,3:4) > 0);
    
    tempFlux(:,3:4) = tempFlux(:,3:4) .* polarityMask;
    ...
    for n = 1:size(tempFlux,1)
        ...
        % First find axial Flux density. We use linear interpolation as we
        % are only using two values, spline requires at least 3 points
        FluxDensity(n,1) = interp1(polys(n,5:6),tempFlux(n,1:2),abs(rzCoords(n,2)));
        ...
        FluxDensity(n,2) = interp1(polys(n,5:6),tempFlux(n,3:4),abs(rzCoords(n,2)));
