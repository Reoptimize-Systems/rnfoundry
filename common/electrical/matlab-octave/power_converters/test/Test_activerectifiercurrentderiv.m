% test_activerectifiercurrentderiv
...
design.MachineSidePowerConverter.Vdc = 1.5 * (3 * sqrt(6) / pi) * peakemfest_ROTARY (design.FluxLinkagePeak * design.CoilsPerBranch, rpm2omega (RPM), design.Poles/2) / sqrt(2);
...
simoptions = simsetup_ROTARY ( design, [], [], ...
                               'EvalFcn', 'test_activerectifiercurrentderiv_evalfcn', ...
                               'Rpm', RPM, ...
                               'TSpan', TSpan, ...
                               'RampPoles', 20, ...
...
simoptions.SkipODEElectricalResults = true;

simoptions.Vsdref = -160;
simoptions.Vsqref = -100;
...
xlabel('Time (s)');
ylabel('Current');
title (sprintf ('Vsdref: %g, Vsqref: %g', simoptions.Vsdref, simoptions.Vsqref));
