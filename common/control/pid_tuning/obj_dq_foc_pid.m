function ObjVal = obj_dq_foc_pid(Chrom, rtn_type, FieldBounds, design, simoptions)
...
% Compute population parameters
   ...
   if Nind == 0
       ...
       if rtn_type == 2
           ObjVal = ['DQ FOC PID Optimisation-' int2str(Dim)];
...
function score = dosimandscoreit (chrom, design, simoptions)
    ...
    design.FOControl.PI_d = pidController ( Kp_d, Ki_d, Kd_d, 'MaxOut', design.MachineSidePowerConverter.Vdc, 'MinOut', -design.MachineSidePowerConverter.Vdc, 'InitialTime', simoptions.FOCTApp );
    design.FOControl.PI_q = pidController ( Kp_q, Ki_q, Kd_q, 'MaxOut', design.MachineSidePowerConverter.Vdc, 'MinOut', -design.MachineSidePowerConverter.Vdc, 'InitialTime', simoptions.FOCTApp );
    
    [T, Y, results, design, simoptions] = simulatemachine_AM ( design, ...
                                                               simoptions );
                        
	if T(end) < simoptions.ODESim.TimeSpan(2)
        score = 1e9;
    else
