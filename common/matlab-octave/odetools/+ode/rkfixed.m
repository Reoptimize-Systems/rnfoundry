...
% rkfixed: fixed step Runge-Kutta integration
% 
% Syntax
%
...
%  dt - Time step
%
%  options - Alternately, one can specify an options structure instead of
%   dt so that this function is compatible with ode45 and its ilk. The only
%   valid fields are MaxStep (the time step) and OutputFcn
%
...
%
%  sol - Alternative output form. Structure containng the following fields:
%
%    x :
%
%    y : 
...
% Modified by Richard Crozier 2017
%
...
c = [ 0, 0.5];
    ...
    if nargout == 1
        t = struct( 'solver', 'ode.rkfixed', ...
                    'x', t.', ...
                    'y', x.' );
                
        t.stats = struct ( 'nsteps', ns-1, ...
                           'nfailed', 0, ...
                           'nfevals', (length(b)+1) * (ns-1) );
