...
%
% 
%     This function can be called with different options/commands to
%     operate on tcp/udp/ip connection in matlab. It supports data transfer
%     with different data types and byte orders. The function is
%     implemented as a MEX-file that needs to be compiled before use.
%
% General syntax
% ==============
%
...
%   
%        pnet(con,'command',....)      or         pnet con command ....
%  
...
%     This function is always nonblocking unless their is some troubles with
...
%     uint8, uint16  and uint32. Arrays of datatype char are written as
%     single bytes, all other datatypes are written as it matlabsize. By
%     default the bytes are swapped to network byte order unless the
%     swapping option is specified. Swapping options:
%
%     'native'  no swapping use the computers native byte order.
%     'swap'    allways swap byte order.
%     'network' uses network byte order (big endian) and it is default.
%     'intel'   use intel CPUs byte order (little endian).
%
%     This operation blocks until writetimeout (is by default inf) is
%     reached or all elements are transfered. Returns number of elements
%     successfull written.
%
...
%     process the format string and following arguments. See help on
%     SPRINTF for more information. It blocks until writetimeout and
%     returns the number of characters succesfully written.
%
%     This command can also be used with UDP packets, see the UDP section.
%     
%  data=pnet(con,'read' [,size] [,datatype] [,swapping] [,'view'] [,'noblock'])
%     
%     Reads an array of elements from a connection. Unless option 'noblock'
%     is used the operation blocks until timeout is reached (by default
%     inf), the specified "size" is reached or the connection is
%     disconnected by peer. Specified "size" (by default 65536) can be a
%     scalar or a vector of specifying dimentions length for the
%     multidimentional array to read. If size is a scalar i.e. [10] it
%     return 0 to 10 elements in a rowvector depending on available
%     elements. If size is a vector i.e. [1 10] or [5 5 5] it returns an
%     array if all elements is available, if not, an empty array is
%     returned. The datatype option is a string containing the name of the
%     datatype, if not specified its by default 'char'. Supported datatypes
%     is the same as for the 'write' command.  The option named "swapping"
%     is is the same for the 'read' command. The option 'view' gives an
%     "preview" of whats available, all data is left in the read buffer. In
%     combination with 'noblock' this is a powerfull command to check how
%     much and whats available at the moment without blocking.
%
%     This command can also be used with UDP packets, see the UDP section.
%     
...
%     returns the number of bytes successfully moved from the connection to
%     the file. If the optional string 'append' is specified the data will
%     be appended to the end of the file, otherwise the file will be
%     overwritten. 'view', 'noblock' and timeout behave as previously
%     described for read operations.
%
%     This command can also be used with UDP packets, see the UDP section.
%     
%  ret=pnet(con,'writefromfile','filename'[[,start] ,len])
%
%     Read data from a file and write it on the connection. All of the file
%     will be transmitted of not "start" and "len" specifies a segment. The
...
%     
%     This returns a number telling about the status of a
%     connection/socket. It is a different number for each type of
%     connection or socket see the #define's in top of pnet.c. The most
%     important status is when peer has disconnected in which case the
%     status value is 0, otherwise it is >0.
%
%     This command can also be used with UDP packets, see the UDP section.
%
%  stat=pnet(con,'setreadtimeout',sec)
%     
%     The value "sec" specifies how long read and listen commands blocks
%     before it timeouts. The unit is seconds specified as a floating point
%     which means that i.e. 0.1 can be specified for a maximum of 0.1
%     seconds blocking. Setting readtimeout to 0 is the same as adding the
%     'noblock' option to all following 'read' or 'listen' calls.
%
%     This command can also be used with UDP packets, see the UDP section.
%
...
%     it timeouts. The unit is seconds specified as a floting point which
%     means that i.e. 0.1 can be specified for a maximum of 0.1 seconds
%     blocking.
%
%     This command can also be used with UDP packets, see the UDP section.
%     
...
%     
%     This command call returns ipnumber and port number for the remote
%     host that is (the latest) associated with the connection/socket.
%     After 'tcplisten' you can get the the clients ip and port numbers.
%     After a 'tcpconnect' you can get the ip and port that you connected
%     to.
%
%     For UDP sockets this gives remote ip and and port for the latest
%     operation of 'readpacket', 'writepacket' (if not connected) or
%     'udpconnect'.
%
%  pnet(con,'close')
%
%     Closes a tcpconnection, tcpsocket or udpsocket. This comand must be
%     called after ending use of any socket or connection, even if it is
%     detected that the connection is broken from the remote host side.
%
%     This command should also be used with UDP sockets, see the UDP section.
%
%  pnet('closeall')
%
...
%     as strings in most cases. This syntax should generally work for all
%     variants of calls and is useful and easy to type when experimenting
...
%
%       Peter Rydes�ter, 
%       Mitth�gskolan(Mid Sweden University) campus �stersund, SWEDEN
%
