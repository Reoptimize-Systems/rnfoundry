...
#define MAX_CON         100       /* Maximum number of simultaneous tcpip connections.*/
...
/********************************************************************/
/* A "wrapper" function for memory allocation. Most for debugging /tracing purpose */
...
{
    if(myoptstrcmp("CHAR",str) == 0)
        return mxCHAR_CLASS;
    if(myoptstrcmp("DOUBLE",str) == 0)
        return mxDOUBLE_CLASS;
    if(myoptstrcmp("SINGLE",str) == 0)
        return mxSINGLE_CLASS;
    if(myoptstrcmp("INT8",str) == 0)
        return mxINT8_CLASS;
    if(myoptstrcmp("UINT8",str) == 0)
        return mxUINT8_CLASS;
    if(myoptstrcmp("INT16",str) == 0)
        return mxINT16_CLASS;
    if(myoptstrcmp("UINT16",str) == 0)
        return mxUINT16_CLASS;
    if(myoptstrcmp("INT32",str) == 0)
        return mxINT32_CLASS;
    if(myoptstrcmp("UINT32",str) == 0)
...
#define SWAPDATA(a,b) { a^= b; b^= a; a^= b; }
    ...
    if(elementsize < 2)
        return;
    if(is_intel_order[0] == 1 && mode == 2)
        mode = 1;
    if(is_intel_order[0] == 0 && mode == 3)
        mode = 1;
    if(mode == 1)
    {
        int e;
...
int my_mexInputArray2Buff(const int argno,io_buff *buff)
{
    ...
    if(my_mexFindInputOption(argno+1, "NATIVE"))
        swap = 0;
    if(my_mexFindInputOption(argno+1, "SWAP"))
        swap = 1;
    if(my_mexFindInputOption(argno+1, "NETWORK"))
        swap = 2;
    if(my_mexFindInputOption(argno+1, "INTEL"))
        swap = 3;
...
int read2buff (const int len, int newline, int noblock)
{
    ...
    {
        /* If enough in buffer then return */
        return len;
    }

    if (0 == IS_STATUS_IO_OK(con[con_index].status))
    {
        /* If not read/write fid (broken pipe) then exit.*/
        if(len < con[con_index].read.pos)
        {
            return len;
        }
    ...
    while (1)
    {
        int left_to_read = len - con[con_index].read.pos;
#ifdef DEBUG
        mexPrintf("DEBUG: read2buff: left_to_read:%d\n",left_to_read);
#endif // DEBUG
        if(left_to_read > 0)
        {
            if(IS_STATUS_CONNECTED(con[con_index].status))
            {
                // attempt to read from connected (tcp) socket
#ifdef DEBUG
                mexPrintf("DEBUG: read2buff: socket is connected (tcp), trying to read %d bytes\n", left_to_read);
#endif // DEBUG
                retval = recv ( con[con_index].fid,
                                &con[con_index].read.ptr[con[con_index].read.pos],
                                left_to_read,
                                MSG_NOSIGNAL );
            }
            else
            {
                // attempt to read from connectionless (udp) socket
                struct sockaddr_in my_addr;
                int fromlen = sizeof(my_addr);

                // Copy 0.0.0.0 address and 0 port to remote_addr as init-value.
...
#ifdef DEBUG
                mexPrintf("DEBUG: read2buff: socket is unconnected (udp), trying to read %d bytes\n", left_to_read);
#endif // DEBUG
                retval = recvfrom ( con[con_index].fid,
                                    &con[con_index].read.ptr[con[con_index].read.pos],
                                    left_to_read,
                                    MSG_NOSIGNAL,
        ...
        left_to_read = retval > 0 ? retval : 0;

        con[con_index].read.pos += left_to_read;
