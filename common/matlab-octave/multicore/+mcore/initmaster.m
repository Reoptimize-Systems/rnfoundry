function [mcstate, settings, evalfcns, postProcStruct, parameters] = initmaster(evalfcns, fcnparams, settings)
% initialisation function for mcore.startmulticoremaster2
    ...
    if settings.debugMode
        ...
        mcstate.startTime    = mcore.mbtime;
...
%     nrOfEvalsAtOnce = settings.nrOfEvalsAtOnce;
    ...
    if settings.clearExistingFiles
        ...
        mcore.deletewithsemaphores2(existingMulticoreFiles);
    end
...
%         curFileNr = mcstate.lastFileNrMaster; % for simpler copy&paste
        ...
        if settings.debugMode, t1 = mcore.mbtime; end
        
        % Generate a file semaphore to prevent workers attempting to access
        % parameter file as we generate it
        sem = mcore.setfilesemaphore(parameterFileName, settings.showWarnings, settings.debugMode, 0, mcstate.dirTimeDiff);
        
        pause(0.01);
        
        if settings.debugMode, mcstate.setTime = mcstate.setTime + mcore.mbtime () - t1; end
        ...
        try
            ...
            if settings.showWarnings
                fprintf('Warning: Unable to save parameter file No. %d at\n%s.\n', curFileNr, parameterFileName);
        ...
        if settings.debugMode, mcstate.removeTime = mcstate.removeTime + mcore.mbtime () - t1; end
