function semaphore = setfilesemaphore2(fileList, showWarnings, debugMode)
        ...
        while 1
            ...
            if ~isempty(dirStruct)
                ...
                for k=1:length(dirStruct)
                    ...
                    elseif isfield(dirStruct, 'datenum')
                        ...
                        fileDatenum = mcore.datenum2(dirStruct(k).date);
                    ...
                    if  (now - fileDatenum) * 86400 <= semaphoreOldTime && ...
                        ...
                        oldSemaphoreFileName = mcore.concatpath(semaphorePatternPath, dirStruct(k).name);
                        ...
                        disp(mcore.textwrap2(sprintf('Ignoring old semaphore of file %s.', fileName)));
                        ...
                        deleteSuccessful = mcore.mbdelete2(oldSemaphoreFileName, showWarnings, 0); %% file access %%
                        
                        if debugMode
                            if deleteSuccessful
                               disp(mcore.textwrap2(sprintf('Successfully deleted old semaphore file: %s.', oldSemaphoreFileName)));
                            else
                               disp(mcore.textwrap2(sprintf('Failed to delete old semaphore file: %s.', oldSemaphoreFileName)));
                            end
                        end
                ...
                if semaphoreExisting && displayWaitInfo && (now - startWaitTime) * 86400 >= waitInfoPeriod
                    disp(mcore.textwrap2(sprintf('Waiting for semaphore of file %s to disappear.', fileName)));
                    if debugMode
                        disp(mcore.textwrap2(sprintf('now - startWaitTime: %f', (now - startWaitTime) * 86400)));
                    end
            ...
            if ~semaphoreExisting
                ...
                for attemptNr = 1:10
                    ...
                    try
                        mcore.generateemptyfile(semaphoreFileName); %% file access %%
                        break
                    catch
                        % in very very very unlikely cases two processes might have
                        % generated the same semaphore file name
                        if showWarnings
                            disp(mcore.textwrap2(sprintf('Warning: An error occured while generating semaphore file %s:', semaphoreFileName)));
