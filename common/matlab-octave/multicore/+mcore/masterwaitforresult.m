function [mcstate, resultCell, sem, resultLoaded] = masterwaitforresult(mcstate, settings, resultCell, sem, paramfname, resultfname, workfname)
...
%
    ...
    while 1
        ...
        if firstRun
            ...
            sem = mcore.setfilesemaphore(paramfname, settings.showWarnings, settings.debugMode, 0, mcstate.dirTimeDiff);
        ...
        % result file.
        if mcore.existfile(resultfname)
            
            % attempt to load the result from result file
            [result, resultLoaded] = mcore.loadResultFile2(resultfname, settings.showWarnings);
...
%                 unexpectedEmptyResult = false;
        ...
        if resultLoaded
            ...
            if mcore.existfile(workfname)
                ...
                jobTimedOut = mcore.mbtime() - mcore.getfiledate(workfname) * 86400 > mcstate.maxMasterWaitTime;
            else
...
%                 unexpectedEmptyResult = false;
            ...
            if settings.debugMode
                mcstate.removeTime = mcstate.removeTime + mcore.mbtime() - t1; 
