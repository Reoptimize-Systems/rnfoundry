...
settings.maxEvalTimeSingle = fEvalTimeSingle * 2;
...
% settings.postProcessHandle   = @mcore.postprocessdemo
...
for n = 1:nrOfRuns
	...
	t0 = mcore.mbtime;
	resultCell = mcore.startmulticoremaster(@testfun, parameterCell, settings);
...
if runWithoutSlaves
  ...
  % Call function STARTMULTICOREMASTER.
  t0 = mcore.mbtime;
  resultCell = mcore.startmulticoremaster(@testfun, parameterCell, settingsTemp);
...
if runDirectly
  ...
  for k = 1:nrOfEvals
    resultCell2{k} = mcore.testfun(parameterCell{k}{:});
  end
