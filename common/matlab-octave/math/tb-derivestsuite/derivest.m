...
%        DEFAULT: 2
%
%   'NominalStep' - Defines the nominal step size to be used by derivest.
%       Useful for periodic functions for which relative step sizes can
%       cause problems.
%
...
if isempty(par.NominalStep)
    par.NominalStep = max(x0,0.02);
else
    par.NominalStep = repmat(par.NominalStep, size(x0));
end
...
% NominalStep = [] by default
if isempty(par.NominalStep)
    par.NominalStep = [];
elseif (length(par.NominalStep)>1) || (par.NominalStep<=0)
  error 'NominalStep must be empty or a scalar, >0.'
end 
