...
%}
    ...
    if isfield(options, 'OutputFcn')
        if ~isempty(options.OutputFcn)
            % Set OutputFcn to be the actual supplied output function
            % which must be called with unconstrained variables
            OutputFcn = options.OutputFcn;
            % Set the options.OutputFcn field to now contain a handle to 
            % the wrapper function
            options.OutputFcn = @OutputFcn_wrapper;
        end
    else
        % create empty OutputFcn field
        options.OutputFcn = [];
    end
    ...
    % check the user-provided input with subfunction check_input
    [funfcn, x0, lb, ub, A, b, Aeq, beq, nonlcon, tolCon, N] = ...
        check_input(funfcn, x0, lb, ub, A, b, Aeq, beq, nonlcon, strictness, tolCon, N);
    ...
    funfncT = @(x) funfncP(X(x, Nzero, Np1zero, lb, ub, lb_ub, lb_only, ub_only, fix_var, unconst), varargin{:});
    ...
    sol = new_x;    
    sol(:) = X(presol, Nzero, Np1zero, lb, ub, lb_ub, lb_only, ub_only, fix_var, unconst);    % with the same size as the original x0
    ...
    [output, exitflag] = finalize(sol, new_x, A, b, Aeq, beq, nonlcon, tolCon, output, exitflag);
    ...
    function stop = OutputFcn_wrapper(x, varargin)
        ...
        x_new(:) = X(x, Nzero, Np1zero, lb, ub, lb_ub, lb_only, ub_only, fix_var, unconst);
    ...
    function [sol, fval, exitflag, output] = NelderMead(funfcn, x0, options, varargin)
        ...
        outputfcnsupplied = false;
        
        if ~isempty(options.OutputFcn) 
            outputfcnsupplied = true; 
            OutputFcn = options.OutputFcn; 
        end
...
function [funfcn, x0, lb, ub, A, b, Aeq, beq, nonlcon, tolCon, N] = ...
    check_input(funfcn, x0, lb, ub, A, b, Aeq, beq, nonlcon, strictness, tolCon, N)
...
function xx = X(x, Nzero, Np1zero, lb, ub, lb_ub, lb_only, ub_only, fix_var, unconst)
...
% finalize the output
function [output, exitflag] = finalize(x, new_x, A, b, Aeq, beq, nonlcon, tolCon, output, exitflag)        
