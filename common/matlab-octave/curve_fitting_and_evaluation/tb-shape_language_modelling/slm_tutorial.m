...
% extrapolating here beyond the data.
...
prescription = slmset(prescription,'maxvalue',1,'minvalue',-1);
slm = slmengine(x,y,prescription);
...
% SLM now has the ability to generate a list of points, then evaluate
...
slm = slmengine(x,y,'incr','on','jerk','positive','concavedown','on','knots',35,'verbosity',1);

%%
% and verbosity of 2 tells slmengine to be more expressive yet.
slm = slmengine(x,y,'incr','on','jerk','positive','concavedown','on','knots',35,'verbosity',2);
