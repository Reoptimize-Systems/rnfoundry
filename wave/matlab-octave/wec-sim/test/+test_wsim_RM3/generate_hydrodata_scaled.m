function hydro = generate_hydrodata_scaled (varargin)
% Run example: hydro = test_wsim_RM3.generate_hydrodata_scaled ('ScaleFac', 2)

    options.PlotBEM = true;
    options.WriteH5 = true;
    options.SkipIfExisting = false;
    options.ScaleFac = 1;
    options.PlotMesh = false;
    options.NemohOutputDir = '';
    
    options = parse_pv_pairs (options, varargin);
    
    dir = getmfilepath ('test_wsim_RM3.generate_hydrodata_scaled');
    
    if isempty (options.NemohOutputDir)
        inputdir = fullfile (dir, 'RM3_NEMOH_output');
    else
        inputdir = options.NemohOutputDir;
    end
	...
	float.scaleMesh (options.ScaleFac);
%     float.scaleMesh (options.ScaleFac, 'CentreOfGravity', [0,0,-10]);
...
%     axis equal;
    ...
    spar.scaleMesh (options.ScaleFac);
...
%     axis equal;
    ...
    % generate the file 
    omega = [0.02, 5.20];
    sim.writeNemoh ( 'NWaveFreqs', 2, ... %260
    ...
    if options.WriteH5
        Write_H5 (hydro, fullfile (dir, 'hydroData'))
    end
