function [mbsys, initptodpos] = make_multibody_system (waves, simu, hydro_mbnodes, hydro_mbbodies, hydro_mbelements, problem_options)
    ...
    % create an orientation which points along the global Z axis 
	...
	% restrain the two nodes to both move along a line attached to the
	% first node which is initially parallel to the Z axis
    ...
    % the next section sets up the forces which are communicated from
    % Matlab to MBDyn. To do this we first need to set up the communication
    % method, and then add a special external structural force object which
    % uses this communication method
    if ispc
    ...
    % create the external structural force which will apply the forces
    % communicated from matlab to the structural nodes which are provided
    % to it, in this case the float and spar nodes
    socket_force = mbdyn.pre.externalStructuralForce ({float_node, spar_node}, [], communicator, ...
                                                      'Labels', 'no', ...
                                                      'Sorted', 'yes', ...
                                                      'Orientation', 'orientation matrix' );

	% set up the type of problem to be solved by MBDyn, in this case an
	% initial value problem it is here where controls on the tolerances of
	% the multibody solution are set along with options on the type of
	% solver to use etc.
